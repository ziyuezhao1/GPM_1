...
def is_rotation_matrix(R):
    ...
    if not np.allclose(np.dot(R, R.T), np.eye(3)):
        return False
...
def om2r(om):
    ...
    for i in range(0,100):
        if is_rotation_matrix(om[i,0,0:3,0:3]):
            pass
        else:
            print("not exact rotation matrix")
    shape = (*om.shape[:-2], 4)
    out = np.zeros(shape, dtype=np.double)
    a=1.0 + om[..., 0, 0] + om[..., 1, 1] + om[..., 2, 2]
    #print("a",a,"om 0 0 ",om[...,0,0].shape,om[..., 0, 0],"om 1 1 ",om[..., 1, 1],"om 2 2 ",om[..., 2, 2], )
    neg_idx = a < 0.00  # 找到小于0的元素的索引
    neg_vals = a[neg_idx]  # 找到小于0的元素
    eps = np.finfo(out.dtype).eps
    print("小于0的元素有：", neg_vals)
    a[neg_idx] = 0  # 将小于0的元素改为0
    print("eps",eps)
    ...
    #eps = np.finfo(out.dtype).eps  # 获取浮点数的最小精度
    #if np.any(out[..., 0] == 0):
        #out[..., 1] = np.where(out[..., 0] == 0, 0, 0.25*(om[..., 2, 1] - om[..., 1, 2])/(out[..., 0] + eps))
        #out[..., 2] = np.where(out[..., 0] == 0, 0, 0.25*(om[..., 0, 2] - om[..., 2, 0])/(out[..., 0] + eps))
        #out[..., 3] = np.where(out[..., 0] == 0, 0, 0.25*(om[..., 1, 0] - om[..., 0, 1])/(out[..., 0] + eps))
...
#### Alignment
##################################################################################
def is_rotation_quaternion(q):
    """
    Check if a quaternion is a valid rotation quaternion.
    """
    # Check if the quaternion is a unit quaternion.
    if not np.isclose(np.linalg.norm(q), 1.0):
        print("the rotation part is not unit",np.linalg.norm(q))
        return False
    # Check if the real part is positive.
    if q[0] < 0:
        print("real part is not positive")
        return False
    return True
...
def calculateBestAligner(estimate, original):
    ...
    print("rotpart_shape",rotpart.shape)
    ...
    print("q norm",0.5 * np.sum(np.diagonal(q @ q.conj().T)))
    if q[0, 0].real < 0:
        q *= -1
    ...
    dq = dq2dqmat(dq)
    print(dqmat2mat(dq).shape)
    print("estimate times tmp", dqmat2mat(dq)[0,0:3,0:3])
    ttt = dqmat2dq(dq)
    print("ttt.shape",ttt.shape)
    print("if the rotation part of q is exact rotation", is_rotation_quaternion(ttt[0, 0:4]))
...
##################################################################################
def get_top_d_left_singular_vectors(A, d):
    """
    Returns the top d left singular vectors of a matrix A.

    Parameters:
        A (numpy.ndarray): Input matrix A nd * nd matrix.
        d (int): Number of left singular vectors to return.

    Returns:
        numpy.ndarray: Array containing the top d left singular vectors of matrix A.
    """
    # Perform singular value decomposition on matrix A
    U, S, Vt = np.linalg.svd(A, full_matrices=False)

    # Get the top d left singular vectors of matrix A
    top_d_left_singular_vectors = U[:, :d]
    top_d_left_singular_vectors = np.real(top_d_left_singular_vectors)
    top_d_left_singular_vectors = top_d_left_singular_vectors / np.linalg.norm(top_d_left_singular_vectors, axis=0)
    m = top_d_left_singular_vectors[0:(d-1), 0:(d-1)]
    if sla.det(m) < 0:
        top_d_left_singular_vectors[:, [0, 1]] = top_d_left_singular_vectors[:, [1, 0]]

    # Return the top d left singular vectors
    return top_d_left_singular_vectors
...
def generalized_power_method(Y, d,k, tol=1e-10, max_iter=20):
    """
    Computes the top k eigenvectors of the matrix A using the power method.
    
    Parameters:
    A (ndarray): The nd times nd matrix to compute the eigenvectors of.
    k (int): The number of eigenvectors to compute.
    tol (float): The tolerance for convergence. Default is 1e-10.
    max_iter (int): The maximum number of iterations. Default is 1000.
    
    Returns:
    eigenvectors (ndarray): A matrix containing the top k eigenvectors as columns.
    """
    # Compute the SVD of A and get top d left singular vectors
    ...
    print("y_translation.shape",Y_translation.shape)
    Y_translation_mat=Y_translation.swapaxes(1, 2).reshape(Y_translation.shape[0]*3, Y_translation.shape[1]*1)
    Y_rotation_mat=mb2bm(Y_rotation,3)
    print(Y_rotation_mat[0:3,0:3])
    print("y_translation_mat.shape",Y_translation_mat.shape)
    
    
    # deal with translation
    
  
    #eigenvalues, eigenvectors = np.linalg.eig(Y_rotation_mat)
    n =Y_rotation_mat.shape[0] // (d-1)
    #idx = eigenvalues.argsort()[::-1][:d-1]
    #Phi = eigenvectors[:,idx]
    #Phi = Phi / np.linalg.norm(Phi, axis=0)
    #Phi = np.sqrt(n) * Phi
   
    U=get_top_d_left_singular_vectors(Y_rotation_mat, d-1)
    ...
    t =0.1*np.ones(Y_rotation_mat.shape[0])
    
    # this is mimic the behavior in algorithm 2: SpectrIn: Spectral Initialization
    # Projection U on SO(d)n
    n = U.shape[0] // (d-1)
    print("U",U[0:6,0:3])# 计算 n 的值
    U_3d = np.reshape(U, (n, 3, 3)) 
    print(U_3d.shape)
    print("U_3D",U_3d[0,:,:])# 将 A 变成 n x 3 x 3 的矩阵
    x =_round_rotations(U_3d)
    print("X",x[0,:,:])
    x_0 = np.reshape(x, (x.shape[0] * 3, 3))
    print("X_0",x_0[0:6,0:3])
    a = np.linalg.norm(x_0 - U, ord="fro")
    print("a",a)
    
    # Projection U on SO(d)n
    n = U1.shape[0] // (d-1)
    print("U1",U1[0:6,0:3])# 计算 n 的值
    U1_3d = np.reshape(U1, (n, 3, 3)) 
    print(U1_3d.shape)
    print("U1_3D",U1_3d[0,:,:])# 将 A 变成 n x 3 x 3 的矩阵
    x1 =_round_rotations(U1_3d)
    print("X1",x1[0,:,:])
    x1_0 = np.reshape(x1, (x1.shape[0] * 3, 3))
    print("X1_0",x1_0[0:6,0:3])
    b = np.linalg.norm(x1_0 - U1, ord="fro")
    print("b",b)
    if a > b:
        x_0=x1_0
    ...
    while err > tol and i < max_iter:
        ...
        y = Y_rotation_mat @ x_0
        
        n = y.shape[0] // (d-1) # 计算 n 的值
        y_3d = np.reshape(y, (n, d-1, d-1))  # 将 A 变成 n x 3 x 3 的矩阵
        z=_round_rotations(y_3d)
        z_0 = np.reshape(z, (z.shape[0] * (d-1), (d-1)))
        
        err = np.linalg.norm(x_0-z_0,ord='fro')
        x_0 = z_0
        i+=1
    Q=random_rotation_matrix(d-1)
    xt= np.hstack((x_0@Q, t.reshape(-1, 1)))
    xt = np.reshape(xt, (n, d-1, d))

    # 在每三行插入一行全零行向量
    new_xt = np.zeros((100,4, 4))
    row = np.zeros((1, xt.shape[2]))
    row[0, -1] = 1
    for i in range(0,100):
        new_xt[i,0:3,0:4]=xt[i,:,:]
        new_xt[i,3,0:4]=row
    print(new_xt[1,:,:])
    new_xt=np.reshape(new_xt, (400, 4))
...
def arrigoni(Y, timeit=False):
    ...
    G = _arrigoni_round(G)
    print("arrigoni",bm2mb(G)[1,0,0:3,0:3])
    if timeit:
...
def _gpm_irls_weights(Y, estimate):
    """
...
def _gpm_applyWeights(Y, weights):
    """
...
def gpm_irls(Y, x0 = None, max_iter = 20,tol=1e-6, irls_max_iter = 15, stopping_tol = 4*1e-3, verbose = False):
    """
    An IRLS scheme using the method of generalized power method.
    """
    ...
    Y_mb = bm2mb(Y.copy())
    if x0 is None:
        previous_estimate = mat2dqmat(bm2mb(generalized_power_method(A, d=d,k=k,tol=tol,max_iter = max_iter)))
    else:
    ...
    for i in range(irls_max_iter):
        weights = _gpm_irls_weights(Y_mb, previous_estimate)
        Y_w = _gpm_applyWeights(Y, weights)
        current_estimate = mat2dqmat(bm2mb(generalized_power_method(A, d=d,k=k,tol=tol,max_iter = max_iter)))
...
def generateGroundTruth(n):
    ...
    print("ground truth",r2om(r).shape)
    print("ground truth",r2om(r)[1,0,:,:])
...
def generateObservations(gt_dqmat, sigma_r, sigma_t, p, q):
    ...
    print("Y_mat",Y_mat[0:4,4:8])

    return Y_dqmat, Y1_mat
...
def calculateError(estimate, original):
    ...
    estimate_mat=dqmat2mat(estimate)
    original_mat=dqmat2mat(original)
    r_estimate=estimate_mat[...,0:3,0:3]
    print(r_estimate.shape)
    r_original=original_mat[...,0:3,0:3]
    print(r_original.shape)
    print(estimate_mat.shape[0])
    r_dist=np.ones(estimate_mat.shape[0])
    rr=r_estimate - r_original
    for i in range(0,estimate_mat.shape[0]):
...
def experiment(gt_dqmat, sigma_r = 1, sigma_t = 1, p = 1, q = 1, dq_max_iter = 20, irls=False, irls_max_iter = 15, verbose = False, timeit=False):
    ...
    print("generate observation",Y_mat.shape)
    print(Y_mat[0:4,4:8])
    ...
    # Calculate estimate using the method of generalized power method
    
    if irls:
        first_estimate_gpm_mat, estimate_gpm_mat = gpm_irls(Y_mat_R, max_iter = dq_max_iter,irls_max_iter = irls_max_iter, verbose = verbose)
        estimate_gpm_mat = bm2mb(estimate_gpm_mat)
        first_estimate_mat = bm2mb(first_estimate_gpm_mat)
    else:
        if timeit:
            gpm_times = {}
            gpm_ev_time_ = time.time()
        estimate_gpm_mat = generalized_power_method(Y_mat, d=4,k=4, max_iter=dq_max_iter)
        if timeit:
            gpm_times["gpm_ev_time"] = time.time() - gpm_ev_time_
        if timeit:
            gpm_times["gpm_rounding_time"] = time.time() - gpm_ev_time_
        
        submatrix = estimate_gpm_mat[4:7, :3] # 取前三行三列组成的小矩阵
        ...
        if np.isclose(det, 1.0):
            ...
            print("小矩阵行列式不为1")
        estimate_gpm_mat = bm2mb(estimate_gpm_mat)
        print("matrix version of gpm", estimate_gpm_mat)
        estimate_gpm_mat = dq2dqmat(rt2dq(*mat2rt(estimate_gpm_mat)))
        if np.isnan(estimate_gpm_mat).any():
            print("estimate包含nan值")
        else:
            print("矩阵estimate不包含nan值")
        #print("checkpoint", estimate_gpm_mat)
    ...
    if irls:
        ...
        first_rerr_gpm_mat, first_terr_gpm_mat = calculateError(first_estimate_gpm_mat, gt_dqmat)
    ...
    rerr_gpm_mat, terr_gpm_mat = calculateError(estimate_gpm_mat, gt_dqmat)

    if irls:
        return rerr_dqmat, terr_dqmat, rerr_mat, terr_mat,rerr_gpm_mat, terr_gpm_mat, first_rerr_dqmat, first_terr_dqmat, first_rerr_mat, first_terr_mat,first_rerr_gpm_mat, first_terr_gpm_mat
    else:
        if not timeit:
            return rerr_dqmat, terr_dqmat, rerr_mat, terr_mat,rerr_gpm_mat, terr_gpm_mat 
        else:
            return rerr_dqmat, terr_dqmat, rerr_mat, terr_mat, dq_times | mat_times | gpm_times
